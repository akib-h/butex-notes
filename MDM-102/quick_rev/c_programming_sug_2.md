...
>
> **Enhanced edition:** every question in Q1–Q6 now has a worked example directly after its explanation.
...
Input: `a = 5`, `b = 5`, `c = 5`
1. Is `a > b`? → `5 > 5` → False.
2. Is `b > c`? → `5 > 5` → False → `largest = c = 5`.
3. Output: **5** — showing the algorithm still resolves correctly even when all values are equal.
...
int main(void) {
    int studentAge = 20;     /* studentAge is an identifier for a variable */
    float _price = 99.5;     /* _price is an identifier starting with underscore */
    char grade1 = 'A';       /* grade1 mixes letters and a digit */

    printf("Age: %d, Price: %.2f, Grade: %c\n", studentAge, _price, grade1);
    return 0;
}
```
**Explanation:** `studentAge`, `_price`, and `grade1` are all identifiers created by the programmer to name the memory locations holding `20`, `99.5`, and `'A'` respectively. `main` and `printf` are also identifiers (naming a function and a library function), while `int`, `float`, and `char` are keywords, not identifiers.
...
```c
int  total;      /* 'int' is a keyword — reserved, fixed meaning */
for (int i = 0; i < 5; i++) {   /* 'for' is a keyword controlling a loop */
    total += i;
}
return 0;        /* 'return' is a keyword */
```
**Explanation:** `int`, `for`, and `return` are keywords — the compiler already knows exactly what each one means and will reject any attempt to reuse them as a variable name (e.g., `int for = 5;` is a compile error).
...
```c
int   age    = 20;     /* whole number  -> data type: int    */
float weight = 65.5;   /* decimal value -> data type: float  */
char  grade  = 'A';    /* single letter -> data type: char   */
```
**Explanation:** Each variable is declared with a data type that tells the compiler how many bytes to reserve and how to interpret those bytes — `age` is stored and read as an integer, `weight` as a floating-point decimal, and `grade` as a single character (internally its ASCII code).
...
int main(void) {
    ...
    char   section     = 'B';
    float  cgpa         = 3.85f;
    double bankBalance  = 254789.6789;

    printf("Roll: %d, Section: %c, CGPA: %.2f, Balance: %.4lf\n",
           rollNumber, section, cgpa, bankBalance);

    return 0;
}
```
**Output:** `Roll: 101, Section: B, CGPA: 3.85, Balance: 254789.6789`
**Explanation:** Each of the four basic data types is used for the kind of value it is best suited to: `int` for a whole roll number, `char` for a single section letter, `float` for a moderately precise CGPA, and `double` for a bank balance that needs more decimal precision.
...
int main(void) {
    const int MAX_STUDENTS = 60;   /* const constant  */
    float radius = 5.0f;
...
```
**Explanation:** `PI` is substituted by the preprocessor before compilation even begins, while `MAX_STUDENTS` is a normal, typed, compiler-checked constant. Both `PI` and `MAX_STUDENTS` will trigger a compiler error if the program later tries to assign a new value to them.
...
int main(void) {
    ...
    printf("Remainder: %d\n", a % b);  /* arithmetic (modulus) operator */

    return 0;
}
```
**Output:** `Sum: 13`, `Is equal: 0`, `Remainder: 1`
**Explanation:** `+` acts on operands `a` and `b` to produce their sum; `==` compares them and returns `0` (false) since `10` is not equal to `3`; `%` returns the remainder of `10 / 3`, which is `1`.
